...
class CharacterActionServiceTest(unittest.TestCase):
    def test_action_to_journal_to_reload(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            reloaded = CharacterState.from_dict(
                PersistenceKernel(tmp, ALLOWED).load_state()["character"]
            )
            self.assertGreater(sum(reloaded.skill_xp.values()), 0)
            self.assertEqual((reloaded.energy, reloaded.stress), (94, 3))
            self.assertEqual(kernel.read_records()[0]["event_type"], "character.resources_changed")
            ...
            self.assertEqual((replay.resolved.character_after.energy, replay.resolved.character_after.stress), (94, 3))
