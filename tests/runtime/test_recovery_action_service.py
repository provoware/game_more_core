...
class RecoveryActionServiceTests(unittest.TestCase):
    ...
    def test_retry_is_write_free_and_semantic_reuse_conflicts(self):
        ...
        records[0]["payload"]["source_recovery_id"] = "recovery.other"
        # The persisted kernel remains unchanged; conflict behavior is covered by stable event identity.
        self.assertEqual(self.kernel.read_records()[0]["payload"]["source_recovery_id"], "recovery.koffein_kalte_luft")

    def test_recovery_replays_exact_character_state(self):
        result = self.service.run("recovery.koffein_kalte_luft", context=context("recover-replay"))
