...
The harness starts the real local server (unless --address is supplied), probes
/api/health and /api/state, and can execute the page in an installed headless
Chrome/Chromium. A browser main-thread freeze therefore becomes a hard timeout
instead of a visually green static-file test.
"""
...
def browser_dom(address: str, *, require_browser: bool, timeout: float = 15.0) -> str | None:
    ...
    if browser is None:
        if require_browser:
            raise RuntimeError(
                "Kein Chrome/Chromium für den echten Browser-Acceptance-Test gefunden"
            )
        return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--no-browser-check",
        action="store_true",
        help="nur /api/health und /api/state prüfen",
    )
    parser.add_argument(
        "--require-browser",
        action="store_true",
        help="fehlschlagen, wenn kein Chrome/Chromium verfügbar ist",
    )
